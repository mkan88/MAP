...
{
	unsigned int seed;
	gsl_rng **rndarray = (gsl_rng **) calloc(gNumOfthreads,sizeof(*rndarray));
...
{	
	...
	// checkCudaErrors(cudaMalloc(&stochasticWeightingDevice, sizeof(double)*pow(6*N, 2)));
	// checkCudaErrors(cudaMalloc(&stochasticWeightingWorkspace, sizeof(double)*pow(6*N, 2)));
